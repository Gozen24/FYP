...
if __name__ == "__main__":
    ...
    if selected == "Price Trend":
        ...
        with col1:
            chosen_state = st.selectbox('Select State', sorted(unique_states1), index=None,placeholder = "Select State...")
        
        with col2:
            category = st.selectbox('Select Food Category', ["Processed Food", "Raw Food"], index=None,placeholder = "Select Category...")
            with col3:
                if category is None:
        ...
        with item:
            ...
            with col2:
                if category is None:
                    ...
                    state=st.selectbox("Select State", unique_states1)
                elif category == "Raw Food":
                    state=st.selectbox("Select State", unique_states2)
